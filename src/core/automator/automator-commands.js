...
export const AutomatorCommands = [
  ...
  },
  {
    ...
    validate: (ctx, V) => {
      ...
      return V.checkBlock(ctx, ctx.If) && (!ctx.variableReference ||
    ...
    compile: (ctx, C) => {
      ...
      const commands = C.visit([ctx.block[0]]);
      const hasElse = ctx.Else !== undefined;
      const elseCommands = hasElse ? C.visit([ctx.block[1]]) : null;
      return {
        run: S => {
  ...
  },
  {
    ...
    validate: (ctx, V) => {
      ...
      return V.validateVariableDeclaration(ctx);
    },
    compile: (ctx, C) => {
