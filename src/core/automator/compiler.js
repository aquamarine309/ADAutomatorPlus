...
import { RESERVED_KEYWORDS } from "./automator-backend.js";
...
class Validator extends BaseVisitor {
  ...
  validateVariableDeclaration(ctx) {
    ...
    if (this.variableTypes.has(varName)) {
      this.addError(ctx,
        ...
        "Cannot redeclare a variable in the global scope.");
      return false;
    }
  ...
  // === Helper Methods ===
  isValidVarName(name) {
    // 1. Check against reserved keywords
    if (RESERVED_KEYWORDS.includes(name.toLowerCase())) {
      return false;
    }
    
    // 2. Validate naming convention
    return /^[a-zA-Z_][a-zA-Z0-9]{0,19}$/.test(name);
  }

  inferExpressionType(exprCtx) {
    ...
    if (exprCtx.StringLiteral || exprCtx.StringLiteralSingleQuote) {
  ...
  checkBlock(ctx, commandToken) {
    let hadError = false;
    if (!ctx.RCurly || ctx.RCurly.some(x => x.isInsertedInRecovery)) {
    ...
    if (!ctx.LCurly || ctx.LCurly.some(x => x.isInsertedInRecovery)) {
...
class Compiler extends BaseVisitor {
  ...
  variableReference(ctx) {
    const varName = ctx.Identifier[0].image;
    return () => {
      if (!(varName in player.reality.automator.variables)) {
        throw new Error(`The variable "${varName}" is not defined`);
      }
      return player.reality.automator.variables[varName];
    };
  }
  ...
  expression(ctx) {
    ...
    if (ctx.StringLiteral || ctx.StringLiteralSingleQuote) {
      ...
      return ctx.AutomatorCurrency[0].tokenType.$getter;
